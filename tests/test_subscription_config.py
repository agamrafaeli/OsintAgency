...
def test_add_subscription_creates_record(memory_db):
    ...
    subscriptions = subscription_config.get_subscriptions(db_path=memory_db)
...
def test_add_subscription_updates_existing(memory_db):
    ...
    subscription_config.add_subscription(
        "@testchannel",
        name="Original Name",
        db_path=memory_db
    )
...
def test_get_subscriptions_returns_all_when_no_filter(memory_db):
    ...
    subscription_config.add_subscription("@active", name="Active", db_path=memory_db)
    subscription_config.add_subscription("@inactive", name="Inactive", db_path=memory_db)
    # Manually deactivate one
    subscription_config.update_subscription(
        "@inactive",
        active=False,
        db_path=memory_db
    )

    all_subscriptions = subscription_config.get_subscriptions(
        active_only=False,
        db_path=memory_db
    )
...
def test_get_subscriptions_filters_active_only(memory_db):
    ...
    active_subscriptions = subscription_config.get_subscriptions(
        active_only=True,
        db_path=memory_db
    )
...
def test_remove_subscription_deletes_record(memory_db):
    ...
    subscription_config.add_subscription("@testchannel", db_path=memory_db)
    result = subscription_config.remove_subscription("@testchannel", db_path=memory_db)
...
def test_remove_subscription_returns_false_for_nonexistent(memory_db):
    ...
    result = subscription_config.remove_subscription("@nonexistent", db_path=memory_db)
